...
* `python-dotenv` пакет который загружает вашей среде разработки переменную среду. Создайте папку `.env` в вашей среде разработки и положите туда свой ключ `TOKEN_SUPERJOB`.
     * Пример данных в папке `.env`
       ```
       TOKEN_SUPERJOB = "Значение вашего API ключа"

       ```
     * В файле уже прописано вверху from dotenv import load_dotenv, а ближе к концу файлов в значении def main():указано load_dotenv и superjob_token = os.environ["TOKEN_SUPERJOB"]. Данное значение создаст переменную среду. Он дает понять Python что необходимые ему TOKEN_SUPERJOB необходимо искать в папке .env и подгрузит его значение в переменную superjob_token.
